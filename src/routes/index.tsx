...
function Index() {
  const dkjCols = [
    { key: "id", label: "ID", className: "w-[140px]", render: (r: DKJRow) => <span className="font-mono">{r.id}{r.isSample && <SampleBadge />}</span> },
    { key: "judulNaskah", label: "Judul", className: "w-[250px]", render: (r: DKJRow) => r.judulNaskah || (r as any).judulnaskah },
    { key: "tahunMenang", label: "Tahun", className: "w-[70px]", render: (r: DKJRow) => r.tahunMenang || (r as any).tahunmenang },
    { key: "posisi", label: "Posisi", className: "w-[120px]", render: (r: DKJRow) => r.posisi },
    { key: "pengarang", label: "Pengarang", className: "w-[150px]", render: (r: DKJRow) => r.pengarang },
    ...
    { key: "paratext", label: "PT", className: "w-[40px] text-center", render: (r: DKJRow) => (r.paratext?.sinopsisPenerbit || r.paratext?.blurb1 || (r.paratext?.rawSynopsisSources && r.paratext.rawSynopsisSources.length > 0)) ? <span className="text-green-500 font-bold">✓</span> : <span className="text-muted-foreground">○</span> },
  ];
